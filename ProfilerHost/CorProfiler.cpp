...
std::wstring CorProfiler::GetModuleDescription(ModuleID moduleId) const
{
    ...
    ComOp(
        this->corProfilerInfo->GetModuleInfo(
            moduleId,
            &baseLoadAddress,
    ...
    ComOp(
        this->corProfilerInfo->GetAssemblyInfo(
            assemblyId,
    ...
    std::wstringstream moduleDesc;
    moduleDesc << assemblyName << L"/" << moduleId << L" : " << moduleFileName;
    return moduleDesc.str();
}

void CorProfiler::OnMethodRegistered(const MethodMetadata& method) const
{
    Logger->info("Registering method {} with server", ToString(method.signature));
    this->ipcClient->RegisterMethod(method);
}

CorProfiler::CorProfiler()
: refCount(0), corProfilerInfo(nullptr)
{
    Logger->info("Connecting to profiling server...");
    this->ipcClient = std::make_unique<IpcClient>();
    Logger->info("Getting profiling config...");
    this->ipcClient->GetProfilingConfig();
    Logger->info("Sucessfully connected to profiling server!");

    this->metadataRegistry.SetMethodRegistrationCallback(
        std::bind(&CorProfiler::OnMethodRegistered, this, std::placeholders::_1));
}
...
HRESULT STDMETHODCALLTYPE CorProfiler::Initialize(IUnknown *pICorProfilerInfoUnk)
{
    HRESULT queryInterfaceResult =
        pICorProfilerInfoUnk->QueryInterface(
            __uuidof(ICorProfilerInfo5),
...
HRESULT STDMETHODCALLTYPE CorProfiler::ModuleLoadFinished(ModuleID moduleId, HRESULT hrStatus)
{
    ...
    try
    {
        Logger->info("Finished loading module {}", ToString(this->GetModuleDescription(moduleId)));
        auto instrumenter = ModuleInstrumenterFactory::Create(&this->metadataRegistry, this->corProfilerInfo, moduleId);
        instrumenter->Instrument();
    }
    catch (_com_error &err)
    {
        LogComError(err, "instrumenting module");
    }
...
HRESULT STDMETHODCALLTYPE CorProfiler::AssemblyLoadFinished(AssemblyID assemblyId, HRESULT hrStatus)
{
    ...
    Logger->info("Finished loading assembly {} with module {}", ToString(assemblyName), moduleId);
...
{
    Logger->info("Thread destroyed: {}", threadId);
    return S_OK;
}

HRESULT STDMETHODCALLTYPE CorProfiler::ThreadAssignedToOSThread(ThreadID managedThreadId, DWORD osThreadId)
{
    Logger->info("Thread {} assigned to OS thread {}", managedThreadId, osThreadId);
    return S_OK;
}
...
{
    Logger->info("Runtime thread suspended: {}", threadId);
    return S_OK;
}

HRESULT STDMETHODCALLTYPE CorProfiler::RuntimeThreadResumed(ThreadID threadId)
{
    Logger->info("Runtime thread resumed: {}", threadId);
    return S_OK;
}
