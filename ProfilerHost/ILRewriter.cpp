    ...
    ULONG64 *hookArg)
{
    ...
    return AddProbe(pilr, onEnterHook, hookArg, pFirstOriginalInstr);
}
    ...
    ULONG64 *hookArg)
{
    ...
    for (ILInstr * pInstr = pilr->GetILList()->m_pNext; pInstr != pilr->GetILList(); pInstr = pInstr->m_pNext)
    {
        ...
        case CEE_RET:
        {
            ...
            hr = AddProbe(pilr, onExitHook, hookArg, pNewRet);
            if (FAILED(hr))
                return hr;
    ...
    ULONG64 *hookArg)
{
    ...
    IfFailRet(rewriter.Import());
    {
        ...
        if (onEnterHook != mdTokenNil)
        {
            IfFailRet(AddEnterProbe(&rewriter, onEnterHook, hookArg));
        }

        if (onExitHook != mdTokenNil)
        {
            IfFailRet(AddExitProbe(&rewriter, onExitHook, hookArg));
        }
    }
